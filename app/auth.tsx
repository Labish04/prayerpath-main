...
export default function AuthScreen() {
      ...
      signupMutation.mutate(
        { name: name.trim(), email: email.trim().toLowerCase(), password },
        {
          onSuccess: async () => {
            ...
            // Map local onboarding state to backend model and synchronize
            const blockedApps = [
            ...
            onboardingMutation.mutate(syncData, {
              onSuccess: () => {
                router.replace('/(tabs)');
              },
              onError: (err) => {
                console.error('Failed to sync onboarding answers:', err);
                router.replace('/(tabs)'); // Still let user in even if sync failed
              }
            });
          },
          onError: (error: any) => {
  ...
  const isLoading = loginMutation.isPending || signupMutation.isPending || onboardingMutation.isPending;
      ...
      >
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ flexGrow: 1, justifyContent: 'center', paddingBottom: 40 }}>
          ...
          <View className="bg-white rounded-3xl p-6 border border-black/5 shadow-sm mb-6">
            {!isLogin && (
              <View className="mb-4">
                <Text className="font-sans text-primary/70 text-sm font-semibold mb-2">Name</Text>
                <View className={`bg-gray-50/50 rounded-2xl border flex-row items-center px-4 h-14 ${errors.name ? 'border-red-500' : 'border-gray-200'}`}>
                ...
                {errors.name && <Text className="font-sans text-red-500 text-xs mt-1 ml-1">{errors.name}</Text>}
              </View>
            )}

            <View className="mb-4">
              <Text className="font-sans text-primary/70 text-sm font-semibold mb-2">Email Address</Text>
              <View className={`bg-gray-50/50 rounded-2xl border flex-row items-center px-4 h-14 ${errors.email ? 'border-red-500' : 'border-gray-200'}`}>
              ...
              {errors.email && <Text className="font-sans text-red-500 text-xs mt-1 ml-1">{errors.email}</Text>}
            </View>

            <View className="mb-6">
              <Text className="font-sans text-primary/70 text-sm font-semibold mb-2">Password</Text>
              <View className={`bg-gray-50/50 rounded-2xl border flex-row items-center px-4 h-14 ${errors.password ? 'border-red-500' : 'border-gray-200'}`}>
              ...
              {errors.password && <Text className="font-sans text-red-500 text-xs mt-1 ml-1">{errors.password}</Text>}
            </View>

            {apiError && (
              <View className="bg-red-50 border border-red-200 rounded-2xl p-4 mb-4 flex-row items-center">
                <Text className="text-red-600 text-sm font-sans flex-1">{apiError}</Text>
              </View>
            )}
          ...
          <View className="flex-row justify-center items-center">
            ...
            <TouchableOpacity onPress={() => { setIsLogin(!isLogin); setErrors({}); }} disabled={isLoading}>
          ...
          {isLogin && (
            <View className="bg-primary/5 border border-primary/10 rounded-2xl p-4 mt-8">
              <Text className="font-sans-bold text-primary text-xs uppercase tracking-wider mb-1">Developer Seed Hint</Text>
              <Text className="font-sans text-primary/70 text-xs leading-relaxed">
                Log in with the seeded user profile to see live analytics instantly:
                {"\n"}• <Text className="font-sans-bold">Email:</Text> jordan@example.com
                {"\n"}• <Text className="font-sans-bold">Password:</Text> password123
              </Text>
            </View>
          )}
          ...
          <TouchableOpacity 
            ...
            disabled={isLoading}
          >
            <Text className="font-sans text-primary/40 text-sm underline">Skip to Dashboard (Offline)</Text>
          </TouchableOpacity>
        </ScrollView>
      </KeyboardAvoidingView>
    </ScreenWrapper>
